...
class TestMemoryConsolidationTypeHandling:
    ...
    async def test_string_arguments_work(self, tmp_path: Path) -> None:
        """Normal case: LLM returns string arguments with valid canonical structure."""
        store = MemoryStore(tmp_path)
        provider = AsyncMock()
        provider.chat = AsyncMock(
            return_value=_make_tool_response(
                history_entry="[2026-01-01] User discussed testing.",
                memory_update=_VALID_MEMORY_TEMPLATE,
            )
        )
    ...
    async def test_dict_arguments_serialized_to_json(self, tmp_path: Path) -> None:
        ...
        Dict memory_update will be JSON-serialized then treated as non-canonical,
        which gets coerced into the Reference Facts section.
        """
        store = MemoryStore(tmp_path)
        provider = AsyncMock()
        provider.chat = AsyncMock(
        ...
        # Dict memory_update is JSON-serialized, which isn't valid canonical structure.
        # Our consolidation rejects malformed updates, so MEMORY.md won't be written.
        # This is expected — the important thing is no TypeError is raised.
